        ...
        if (session.getColumnFamilyType() == ColumnFamilyType.Super)
        {
            ...
            for (int j = 0; j < session.getSuperColumns(); j++)
            {
                ...
                for (int t = 0; t < session.getRetryTimes(); t++)
                {
                    ...
                    try
                    {
                        //HL: to make it coherent with COPS-T multiget_slice definition, we put 0 as readTS for now
                        //Maybe need to modify this later when we get to know what this is actually testing.
                        results = client.multiget_slice(keys, parent, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED, 0L, 0L).value;
