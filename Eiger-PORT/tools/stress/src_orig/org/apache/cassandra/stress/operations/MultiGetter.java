        ...
        if (session.getColumnFamilyType() == ColumnFamilyType.Super)
        {
            ...
            for (int j = 0; j < session.getSuperColumns(); j++)
            {
                ...
                for (int t = 0; t < session.getRetryTimes(); t++)
                {
                    ...
                    try
                    {
                        // Haonan: set readTS to 0, b/c our project should not call multigetter
                        results = client.multiget_slice(keys, parent, predicate, session.getConsistencyLevel(), LamportClock.COPS_UNSUPPORTED, 0L).value;
