...
public class CassandraServer implements Cassandra.Iface
{
    ...
    private final static List<IColumn> EMPTY_COLUMNS = Collections.emptyList();
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        if (thriftify) {
            ...
            {
                ColumnFamily cf = columnFamilies.get(StorageService.getPartitioner().decorateKey(command.key));
                if (cf != null && cf.getSortedColumns() != null) {
    ...
    @Override
    public MultigetSliceResult multiget_slice(List<ByteBuffer> keys, ColumnParent column_parent, SlicePredicate predicate, ConsistencyLevel consistency_level, long lts, long read_ts, long client_id)
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        LamportClock.updateTime(lts);
        logger.debug("multiget_slice for new rotxn");
        assert(read_ts >= 0);
        assert(client_id != 0);

        // put nodeip and readts into gc_map
        long nodeIP = (client_id >> 32) & 0xFFFFFFFF;
        LamportClock.setReadTS(nodeIP, read_ts);
        ...
        //pendingTransactions for now is always null -- pending wtxns don't matter to us.
        selectChosenResults(keyToColumnFamily, predicate, read_ts, client_id, keyToChosenColumns, pendingTransactionIds);
        return new MultigetSliceResult(keyToChosenColumns, LamportClock.sendTimestamp(), LamportClock.local_safe_time);
    }


    private void selectChosenResults(Map<ByteBuffer, Collection<IColumn>> keyToColumnFamily, SlicePredicate predicate, long chosen_time, long client_id, Map<ByteBuffer, List<ColumnOrSuperColumn>> keyToChosenColumns, Set<Long> pendingTransactionIds)
    {
        for(Entry<ByteBuffer, Collection<IColumn>> entry : keyToColumnFamily.entrySet()) {
            ...
            for (IColumn column : columns) {
                ChosenColumnResult ccr = ThriftConverter.selectChosenColumn(column, chosen_time, client_id);
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException, TException
    {
        ...
        Map<ByteBuffer, List<ColumnOrSuperColumn>> keyToChosenColumns = new HashMap<ByteBuffer, List<ColumnOrSuperColumn>>();
        Set<Long> pendingTransactionIds = new HashSet<Long>();
        selectChosenResults(keyToColumnFamily, predicate, chosen_time, -1, keyToChosenColumns, pendingTransactionIds);
        ...
        if (pendingTransactionIds.size() > 0) {
            do {
                ...
                try {
                    pendingTransactionIds = TransactionProxy.checkTransactions(state().getKeyspace(), pendingTransactionIds, chosen_time);
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
            ...
            iSliceMap = multigetSliceInternal(state().getKeyspace(), keys, column_parent, predicate, consistency_level, false);
            keyToColumnFamily = ((InternalSliceMap) iSliceMap).cassandraMap;
            selectChosenResults(keyToColumnFamily, predicate, chosen_time, -1, keyToChosenColumns, pendingTransactionIds);
        ...
        // HL: put a 0L as a place holder, we never use get by time function any more
        return new MultigetSliceResult(keyToChosenColumns, LamportClock.sendTimestamp(), 0L);
    }
...
//        return new MultigetCountResult(results, LamportClock.sendTimestamp());
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        return new BatchMutateResult(new_deps, LamportClock.sendTimestamp(), -1);
    }
    ...
    @Override
    public short transactional_batch_mutate_cohort(Map<ByteBuffer, Map<String, List<Mutation>>> mutation_map, ByteBuffer coordinator_key, long transaction_id, long lts, long client_id, long gst)
    throws InvalidRequestException, TException
    {
        LamportClock.updateTime(lts);
        ...
        cohort.receiveLocalTransaction(keyspace, mutation_map, coordinator_key, transaction_id, client_id, gst);

        return 0;
    }

    @Override
    public BatchMutateResult transactional_batch_mutate_coordinator(Map<ByteBuffer, Map<String, List<Mutation>>> mutation_map, ConsistencyLevel consistency_level, Set<Dep> deps, ByteBuffer coordinator_key, Set<ByteBuffer> all_keys, long transaction_id, long lts, long client_id, long gst)
    throws InvalidRequestException, UnavailableException, TimedOutException, TException
    {
        LamportClock.updateTime(lts);
        ...
        try {
            coordinator.receiveLocalTransaction(keyspace, mutation_map, dependencies, coordinator_key, all_keys, transaction_id, client_id, gst);
        } catch (IOException e) {
            InvalidRequestException ex = new InvalidRequestException(e.getMessage());
            ex.initCause(e);
            throw ex;
        ...
        Set<Dep> new_deps;
        if (timestamp != IColumn.VERSION_NOT_FOUND) {
            new_deps = Collections.singleton(new Dep(originalCoordinatorKey, timestamp));
        } else {
            new_deps = Collections.EMPTY_SET;
        }

        if (logger.isTraceEnabled()) {
            logger.trace("transactional_batch_mutate_coordinator({}, {}, {}, {}) = {}", new Object[]{mutation_map, consistency_level, deps, lts, new_deps});
        }
        return new BatchMutateResult(new_deps, LamportClock.sendTimestamp(), LamportClock.local_safe_time);
    }
