...
public class RowMutation implements IMutation, MessageProducer
{
    ...
    public static RowMutation hintFor(RowMutation mutation, ByteBuffer token) throws IOException
    {
        ...
        rm.add(path, ByteBuffer.wrap(mutation.getSerializedBuffer(MessagingService.version_)), System.currentTimeMillis(), System.currentTimeMillis(), ttl, null, -1, -1);

        // serialization version
        path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId, ByteBufferUtil.bytes("version"));
        rm.add(path, ByteBufferUtil.bytes(MessagingService.version_), System.currentTimeMillis(), System.currentTimeMillis(), ttl, null, -1, -1);

        // table
        path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId, ByteBufferUtil.bytes("table"));
        rm.add(path, ByteBufferUtil.bytes(mutation.getTable()), System.currentTimeMillis(), System.currentTimeMillis(), ttl, null, -1, -1);

        // key
        path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId, ByteBufferUtil.bytes("key"));
        rm.add(path, mutation.key(), System.currentTimeMillis(), System.currentTimeMillis(), ttl, null, -1, -1);
    ...
    {
        add(path, value, timestamp, timestamp, timeToLive, null, -1, -1);
    }


    public void add(QueryPath path, ByteBuffer value, long timestamp, long earliestValidTime, int timeToLive, ByteBuffer transactionCoordinatorKey, long client_id, long gst)
    {
        ...
        columnFamily.addColumn(path, value, timestamp, earliestValidTime, timeToLive, transactionCoordinatorKey, client_id, gst);
    }
    ...
    {
        add(path, value, timestamp, timestamp, 0, null, -1, -1);
    }
    ...
    @Override
    public void updateTimestamps(long timestamp, long localCommitTime, boolean clearCoordinatorKey)
    {
        for (ColumnFamily columnFamily : modifications_.values()) {
            for (IColumn column : columnFamily.columns) {
                if (column instanceof SuperColumn) {
                    ...
                    for (IColumn subcolumn : column.getSubColumns()) {
                        subcolumn.setTimestamp(timestamp);
                        subcolumn.setEarliestValidTime(localCommitTime);
                        if (clearCoordinatorKey) {
                            subcolumn.setTransactionCoordinatorKey(null);
                        }
                    }
                } else if (column instanceof DeletedColumn) {
                    assert column.getSubColumns().size() == 0 : "Haven't added support for deleting supercolumn in transactions yet";
                    ...
                    column.setEarliestValidTime(localCommitTime);
                    if (clearCoordinatorKey) {
                        column.setTransactionCoordinatorKey(null);
                    }
                } else {
                    assert column instanceof Column;
                    column.setTimestamp(timestamp);
    ...
    @Override
    public PendingTransactionMutation convertToPending(long pendingTime, long transactionId, ByteBuffer transactionCoordinatorKey)
    {
            ...
            for (IColumn column : columnFamily.columns) {
                if (column instanceof SuperColumn) {
                    if (column.getSubColumns().size() == 0) {
                        ...
                        pendingMutation.makePending(path, transactionCoordinatorKey);
                    } else {
                        for (IColumn subcolumn : column.getSubColumns()) {
    ...
    {
        addColumnOrSuperColumn(cfName, cosc, timestamp, localCommitTime, null, -1, -1);
    }

    public void addColumnOrSuperColumn(String cfName, ColumnOrSuperColumn cosc, long timestamp, long localCommitTime, ByteBuffer transactionCoordinatorKey, long client_id, long gst)
    {
        if (cosc.super_column != null)
        {
            for (org.apache.cassandra.thrift.Column column : cosc.super_column.columns)
            {
                ...
                add(new QueryPath(cfName, cosc.super_column.name, column.name), column.value, timestamp, localCommitTime, column.ttl, transactionCoordinatorKey, -1, -1);
            }
        }
        else if (cosc.column != null)
        {
            ...
            add(new QueryPath(cfName, null, cosc.column.name), cosc.column.value, timestamp, localCommitTime, cosc.column.ttl, transactionCoordinatorKey, client_id, gst);
        }
