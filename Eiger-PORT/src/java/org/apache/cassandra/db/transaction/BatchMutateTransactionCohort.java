...
//    WAIT_FOR_COMMIT -- receive commit --apply batch mutate -- send ack to coordinator -- cleanup
    ...
    public void receiveLocalTransaction(String keyspace, Map<ByteBuffer, Map<String, List<Mutation>>> mutation_map, ByteBuffer coordinator_key, long transaction_id, long client_id, long gst)
	...
	if (sanityCheckFireOnces) {
        ...
        if (local) {
            try {
                ...
                //TransactionProxy.replicateTransactionToOtherDatacentersOneWay(this, keyspace, aLocalKey);
            } catch (IOException e) {
		e.printStackTrace();
                throw new RuntimeException(e);
            }
    ...
    public static BatchMutateTransactionCohort fromBytes(Message originatingMessage, String id) throws IOException
    {
        BatchMutateTransactionCohort cohort = serializer().deserialize(new DataInputStream(new FastByteArrayInputStream(originatingMessage.getMessageBody())), originatingMessage.getVersion());
        cohort.originatingMessage = originatingMessage;
        cohort.id = id;

        if (!cohort.local) {
            TransactionProxy.sendReplicationResponse(originatingMessage, id, cohort.keyspace, cohort.mutations.iterator().next().key());
        }
        return cohort;
    }
