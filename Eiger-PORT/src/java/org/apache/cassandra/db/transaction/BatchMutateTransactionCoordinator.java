...
public class BatchMutateTransactionCoordinator implements MessageProducer
{
    ...
    // HL: for tracking local safe time
    private long pending_time = -1L;
    ...
    // original message that replicates this coordinator
    // null if it is originating from local
    private Message originatingMessage = null;
    ...
    public void receiveLocalTransaction(String keyspace, Map<ByteBuffer, Map<String, List<Mutation>>> mutation_map, Set<Dependency> deps, ByteBuffer coordinator_key, Set<ByteBuffer> all_keys, long transaction_id, long client_id, long gst)
            throws InvalidRequestException, IOException, TimeoutException
    {
        ...
        // HL: get the pending time and record it.
        this.pending_time = LamportClock.record_pending_time();

        List<IMutation> mutations = BatchMutateTransactionUtil.convertToInternalMutations(keyspace, mutation_map, coordinator_key, client_id, gst);
    ...
    public void receiveReplicatedTransaction(String keyspace, List<IMutation> mutations, Set<Dependency> deps, ByteBuffer coordinator_key, Set<ByteBuffer> all_keys, long transaction_id, long timestamp)
    {
        ...
        AppliedOperations2.addPendingOp(coordinator_key, timestamp);
        ...
        StorageProxy.checkGroupDependencies(keyspace, coordinatorKey, timestamp, deps, new TransactionDepCheckCompletable());
        checkQueuedNotifications();
         */
    }

    public void onReceivingReplicatedTransaction() {
        state = CoordinatorState.WAIT_FOR_DEPS_AND_NOTIFICATIONS;
        StorageProxy.checkGroupDependencies(keyspace, coordinatorKey, timestamp, deps, new TransactionDepCheckCompletable());
        checkQueuedNotifications();
    }
    ...
    private void commit()
    {
        ...
        try {
            if (local) {
                BatchMutateTransactionUtil.markTransactionPending(keyspace, mutations, transactionId, coordinatorKey);
            } else {
                assert timestamp != null : "Timestamp must be set from the originating DC";
                BatchMutateTransactionUtil.markTransactionPending(keyspace, mutations, transactionId, timestamp, coordinatorKey);
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(e);
        ...
        AppliedOperations2.addAppliedOp(coordinatorKey, timestamp);
        ...
        // HL: now wotxn committed, we update pending set
        if (local) {
            // for now, we only do local wotxns, TODO: take into account replicated wotxns.
            /*
            assert LamportClock.running_write_txns.containsKey(pending_time) : "We should have recorded this wotxn id: " +
                    transactionId + "; pending time: " + pending_time + "; why missing?";
            assert pending_time != -1 : "Pending_time should have been set!";
            */
            LamportClock.remove_pending_time(pending_time);

        }

        // send reply to originating datacenter.
        // this is for backpressure
        /*
        if (!local) {
            TransactionProxy.sendReplicationResponse(originatingMessage, id, keyspace, coordinatorKey);
        }
        */
    ...
    public static BatchMutateTransactionCoordinator fromBytes(Message originatingMessage, String id) throws IOException
    {
        BatchMutateTransactionCoordinator coordinator = serializer().deserialize(new DataInputStream(new FastByteArrayInputStream(originatingMessage.getMessageBody())), originatingMessage.getVersion());
        coordinator.originatingMessage = originatingMessage;
        coordinator.id = id;

        if (!coordinator.local) {
            TransactionProxy.sendReplicationResponse(originatingMessage, id, coordinator.keyspace, coordinator.coordinatorKey);
        }
        coordinator.onReceivingReplicatedTransaction();
