...
public class BatchMutateTransactionUtil
{
    ...
    public static List<IMutation> convertToInternalMutations(String keyspace, Map<ByteBuffer, Map<String, List<Mutation>>> mutation_map, ByteBuffer coordinatorKey, long client_id, long gst)
        ...
        for (Map.Entry<ByteBuffer, Map<String, List<Mutation>>> mutationEntry: mutation_map.entrySet()) {
            ...
            for (Map.Entry<String, List<Mutation>> columnFamilyMutations : columnFamilyToMutations.entrySet()) {
                ...
                for (Mutation mutation : columnFamilyMutations.getValue())
                {
                    ...
                    {
                        rm.addColumnOrSuperColumn(cfName, mutation.column_or_supercolumn, timestamp, localCommitTime, coordinatorKey, client_id, gst);
                    }
                }
            }
    ...
    public static void markTransactionPending(String keyspace, List<IMutation> mutations, long transactionId, ByteBuffer transactionCoordinatorKey)
    throws InvalidRequestException, IOException
    {
        long pendingTime = LamportClock.getVersion();

        for (IMutation mutation : mutations) {
            insertLocal(mutation.convertToPending(pendingTime, transactionId, transactionCoordinatorKey));
        }
    }

    public static void markTransactionPending(String keyspace, List<IMutation> mutations, long transactionId, long timestamp, ByteBuffer transactionCoordinatorKey)
            throws InvalidRequestException, IOException {
        long pendingTime = LamportClock.getVersion();

        for (IMutation mutation : mutations) {
            PendingTransactionMutation pendingMutation = mutation.convertToPending(pendingTime, transactionId, transactionCoordinatorKey);
            pendingMutation.updateTimestamps(timestamp, pendingTime);
            insertLocal(pendingMutation);
        }
    }
    ...
    {
        synchronized (checkedTransactions) {
            for (Entry<Long, Long> entry : transactionIdToResult.entrySet()) {
                long transactionId = entry.getKey();
                Long commitTime = entry.getValue();

                // when compare Long object: SHOULD use equals()
                // Java cache Integer objects whose values range from -128 to 127
                if (!commitTime.equals(Long.MIN_VALUE)) {
    ...
    public static boolean isTransactionCommitted(Long transactionId) {
        synchronized (checkedTransactions) {
            CommitOrNotYetTime conyt = checkedTransactions.get(transactionId);
            return (conyt != null && conyt.commitTime != null);
        }
    }
}
