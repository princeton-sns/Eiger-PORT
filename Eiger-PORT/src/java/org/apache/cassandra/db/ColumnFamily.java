...
public class ColumnFamily extends AbstractColumnContainer
{
    ...
    {
        addColumn(path, value, timestamp, timestamp, 0, null, -1, -1);
    }

    public void addColumn(QueryPath path, ByteBuffer value, long timestamp, int timeToLive)
    {
        addColumn(path, value, timestamp, timestamp, timeToLive, null, -1, -1);
    }

    public void addColumn(QueryPath path, ByteBuffer value, long timestamp, long earliestValidTime, int timeToLive, ByteBuffer transactionCoordinatorKey, long client_id, long gst)
    {
        ...
        // HL: record client id and gst
        column.setClientId(client_id);
        column.setGST(gst);
...
//            columnFamily.addColumn(sc);
    ...
    public void addPendingTransactionColumn(QueryPath path, long transactionId, long pendingTime, ByteBuffer transactionCoordinatorKey)
    {
        if (path.superColumnName == null && path.columnName == null)
        {
            assert false : "deleting entire columnFamilies in a transction is not yet supported";
        ...
        {
            assert false : "deleting SuperColumns in a transaction is not yet supported";
        }

        PendingTransactionColumn column = new PendingTransactionColumn(path.columnName, transactionId, pendingTime, transactionCoordinatorKey);
