...
public class FBUtilities
{
    ...
    public static <T> byte[] serialize(T object, IVersionedSerializer<T> serializer, int version) throws IOException
    {
        ...
        // Khiem's note TODO: handle this more elegantly
        // This assertion, in some rear cases, is not true. Problem is TOC/TOU
        // It is possible that some member of objects (e.g., list) is updated between
        // checking the size+allocating buffer and serializing the object
        /*
        ...
        if (!(buffer.getLength() == size && buffer.getData().length == size)) {
            logger_.info(String.format("Final buffer length %s to accommodate data size of %s (predicted %s) for %s",
                    buffer.getData().length, buffer.getLength(), size, object));
            return (new DataOutputBuffer(0)).getData();
        }
