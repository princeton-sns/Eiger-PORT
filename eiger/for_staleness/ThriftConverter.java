   ...
   public static ChosenColumnResult selectChosenColumn(IColumn column, long chosenTime)
   {
       ...
       synchronized(column){
	       ...
	       logger.info("xxstalenessxx: " + 0);
               ...
               if (column.previousVersions() != null) {
		   Iterator<IColumn> itr = column.previousVersions().iterator();
                   IColumn lastCol = null;
                   while (itr.hasNext()) {
                       IColumn oldColumn = itr.next();
                       if (oldColumn.earliestValidTime() <= chosenTime) {
                           ...
                           if (lastCol == null) {
                               logger.info("xxstalenessxx: " + (System.currentTimeMillis() - column.creatTime()));
                           } else {
                               logger.info("xxstalenessxx: " + (System.currentTimeMillis() - lastCol.creatTime()));
                           }
