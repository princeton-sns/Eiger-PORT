...
*/
    ...
    public Map<ByteBuffer, List<ColumnOrSuperColumn>> transactional_multiget_slice(List<ByteBuffer> allKeys, ColumnParent column_parent, SlicePredicate predicate, CopsTestingConcurrentWriteHook afterFirstReadWriteHook, CopsTestingConcurrentWriteHook afterFirstRoundWriteHook)
        ...
        if (lastTime == null) {
            ...
            //System.err.println("numFirstRounds -- numFirstRoundRPCs -- numSecondRounds -- numSecondRoundKeys -- numSecondRoundRPCs");
        }
        ...
        if (COLLECT_STATS) {
            numFirstRounds++;
            numFirstRoundRPCs += asyncClientToFirstRoundKeys.entrySet().size();
        }

        //Send Round 1 Requests
        boolean firstRoundSuccess = true;
        Exception firstRoundException = null;
        ...
        for (Entry<Cassandra.AsyncClient, List<ByteBuffer>> entry : asyncClientToFirstRoundKeys.entrySet()) {
            ...
            if (asyncClient.hasError()) {
                System.err.println("ROT-1: AAA! Why do I get this error: " + asyncClient.getError());
                asyncClient = createNewAsyncClient(keysForThisClient.get(0));
                // Create new client to replace the failed one
            }
        ...
        List<ByteBuffer> pendingKeys = new ArrayList<ByteBuffer>();
        for (BlockingQueueCallback<multiget_slice_call> callback : firstRoundCallbacks) {
            try {
                ...
                for (Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : result.value.entrySet()) {
                    ...
                    for (ColumnOrSuperColumn cosc : coscList) {
                        ...
                        if (evtAndLvt.getEarliestValidTime() == Long.MIN_VALUE && !pendingKeys.contains(key)) {
                            pendingKeys.add(key);
                        }
                        //find the evt and lvt for the entire row
        ...
        if (!firstRoundSuccess) {
            throw firstRoundException;
        }
        //if (logger.isTraceEnabled()) { logger.trace("Min LVT:" + overallMinLvt + "  Max EVT: " + overallMaxEvt); }

        //Execute 2nd round if necessary
        boolean isSecondRound = (overallMaxEvt > overallMinLvt || pendingKeys.size() > 0);
        boolean secondRoundSuccess = true;
        Exception secondRoundException = null;
        if (isSecondRound) {
            //get the smallest lvt > maxEvt
            //long chosenTime = lvtToKeys.navigableKeySet().higher(overallMaxEvt);
            Long chosenTime = lvtToKeys.navigableKeySet().ceiling(overallMaxEvt);

            List<ByteBuffer> secondRoundKeys = new LinkedList<ByteBuffer>();
            if (chosenTime != null && lvtToKeys.headMap(chosenTime) != null) {
            ...
            // Add pending keys
            for (ByteBuffer key : pendingKeys) {
                if (!secondRoundKeys.contains(key)) {
                    secondRoundKeys.add(key);
                }
            }
            ...
            // Khiem: collects stats
            if (COLLECT_STATS) {
                numSecondRounds++;
                numSecondRoundRPCs += asyncClientToSecondRoundKeys.entrySet().size();
                numSecondRoundKeys += secondRoundKeys.size();
            }
            ...
            for (Entry<Cassandra.AsyncClient, List<ByteBuffer>> entry : asyncClientToSecondRoundKeys.entrySet()) {
                ...
                if (asyncClient.hasError()) {
                    System.err.println("ROT-2: AAA! Why do I get this error: " + asyncClient.getError());
                    asyncClient = createNewAsyncClient(keysForThisClient.get(0));
                }
            ...
            for (BlockingQueueCallback<multiget_slice_by_time_call> callback : secondRoundCallbacks) {
                try {
                    ...
                    for (Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : result.value.entrySet()) {
                        ...
                        if (keyToResult.containsKey(key) && coscList.size() >= keyToResult.get(key).size()) {
                            keyToResult.put(key, coscList);
                        }
                    }
            ...
            if (!secondRoundSuccess) {
                throw secondRoundException;
            }
        ...
        for (Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : keyToResult.entrySet()) {
            ...
            for (Iterator<ColumnOrSuperColumn> cosc_it = coscList.iterator(); cosc_it.hasNext(); ) {
                ColumnOrSuperColumn cosc = cosc_it.next();
                try {
                    if (cosc.column.timestamp == IColumn.VERSION_NOT_FOUND || ByteBufferUtil.compareUnsigned(cosc.column.value, ByteBufferUtil.EMPTY_BYTE_BUFFER) == 0) {
                        continue;
                    }
                    clientContext.addDep(key, cosc);
                } catch (NotFoundException nfe) {
        ...
        if (COLLECT_STATS && ((System.currentTimeMillis() - lastTime) >= PRINT_STATS_INTERVAL || (numFirstRounds % 100 == 0))) {
//            printStats();
            lastTime = System.currentTimeMillis();
        }
