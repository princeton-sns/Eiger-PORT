...
public class StorageProxy implements StorageProxyMBean {
    ...
    private static AtomicInteger numLocalRounds = new AtomicInteger(0);
    private static AtomicInteger numLocalReads = new AtomicInteger(0);
    ...
    // Blocking queue to control how fast we should replicate writes
    public static boolean FLOW_CONTROL = false;
    //public static BlockingQueue<Boolean> outstandingOps = new LinkedBlockingQueue<Boolean>(10000);
    public static BlockingQueue<Boolean> outstandingOps = new LinkedBlockingQueue<Boolean>(1000);

    private StorageProxy() {
    }
/*
    static class PrintStats extends TimerTask {
        public void run() {
            printStats();
        }
    }

    private static TimerTask timerTask = new PrintStats();
...
*/
  ...
  //          timer.scheduleAtFixedRate(timerTask, 10, 5000);
    ...
    public static void sendToHintedEndpoints(final RowMutation rm,
        ...
        ReplicationCompletion replicationCompletion = new ReplicationCompletion();
        ReplicationCallback replicationCallback = new ReplicationCallback(targets.size() - 1, replicationCompletion);

        if (targets.size() > 1) {
            try {
                numReplicationsSent.incrementAndGet();
                if (FLOW_CONTROL) {
                    outstandingOps.put(true);
                }
        ...
        sendMessages(localDataCenter, dcMessages, replicationCallback);
    }
     ...
     */
    public static void sendMessages(String localDataCenter, Map<String, Multimap<Message, InetAddress>> dcMessages, IAsyncCallback handler)
            throws IOException {
    ...
    private static List<Row> fetchRows(List<ReadCommand> initialCommands, ConsistencyLevel consistency_level) throws IOException, UnavailableException, TimeoutException {
        ...
        do {
            ...
            numLocalRounds.incrementAndGet();
            ...
            for (int i = 0; i < commands.size(); i++) {
                ...
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("OOPS! Something wrong: {}", ex.toString());
                    }
                    continue;
                }
            }
    ...
    */
    ...
    public static void checkGroupDependencies(final String keyspace, final ByteBuffer locatorKey, final long timestamp, final Set<Dependency> deps, final ICompletable completable) {
        if (logger.isDebugEnabled())
            logger.debug("Checking deps for " + completable);

        AppliedOperations2.addPendingOp(locatorKey, timestamp);

        // Group dependencies by endpoint
        Map<InetAddress, Set<Dependency>> groupDeps = groupDependenciesByEndpoint(keyspace, deps);

        //Send out all group_dep_checks in parallel
        final GroupDepCheckCallback groupDepCheckCallback = new GroupDepCheckCallback(groupDeps, completable);

        for (final Map.Entry<InetAddress, Set<Dependency>> e : groupDeps.entrySet()) {
            if (e.getKey().equals(FBUtilities.getBroadcastAddress()) && OPTIMIZE_LOCAL_REQUESTS) {

                Runnable runnable = new WrappedRunnable() {
                    @Override
                    protected void runMayThrow() throws Exception {
                        AppliedOperations2.checkGroupDependency(new GroupDependencyCheck(e.getValue()), null, null, groupDepCheckCallback);
                    }
                };
                ...
                Runnable runnable = new WrappedRunnable() {
                    @Override
                    protected void runMayThrow() throws Exception {
                        MessagingService.instance().sendRR(new GroupDependencyCheck(e.getValue()), e.getKey(), groupDepCheckCallback);
                    }
                };
                StageManager.getStage(Stage.DEPENDENCY_CHECK).execute(runnable);
            }
    ...
    public static Map<InetAddress, Set<Dependency>> groupDependenciesByEndpoint(final String keyspace, final Set<Dependency> deps) {

        Map<InetAddress, Set<Dependency>> depGroups = new HashMap<InetAddress, Set<Dependency>>();
        for (Dependency dep : deps) {
            List<InetAddress> localEndpoints = StorageService.instance.getLocalLiveNaturalEndpoints(keyspace, dep.getLocatorKey());
            assert localEndpoints.size() == 1 : "Assumed for now";
            InetAddress localEndpoint = localEndpoints.get(0);

            Set<Dependency> depGroup = depGroups.get(localEndpoint);
            if (depGroup == null) {
                depGroup = new HashSet<Dependency>();
                depGroups.put(localEndpoint, depGroup);
            }
            depGroup.add(dep);
        }
    ...
    public static void printStats() {
        if (logger.isInfoEnabled()) {
            logger.info("Number of local_rounds/local_reads: " + numLocalRounds.get() + " " + numLocalReads.get());

            if (numReplicationsSent.get() > 0) {
                logger.info("REPLICATION: sent = " + numReplicationsSent.get() +
                        "; received = " + numReplicationsReceived.get() +
                        "; ratio = " + numReplicationsReceived.get() * 1.0 / numReplicationsSent.get());
            }

            if (TransactionProxy.numReplicationsSent.get() > 0) {
                logger.info("WOT: sent = " + TransactionProxy.numReplicationsSent.get() +
                        "; received = " + TransactionProxy.numReplicationsReceived.get() +
                        "; ratio = " + TransactionProxy.numReplicationsReceived.get() * 1.0 / TransactionProxy.numReplicationsSent.get());
            }
        }
    }
}
