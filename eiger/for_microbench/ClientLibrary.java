...
public class ClientLibrary {
	...
	for (int i = 0; i < 1; ++i) {
        ...
        for (Entry<Cassandra.AsyncClient, List<ByteBuffer>> entry : asyncClientToFirstRoundKeys.entrySet()) {
            ...
            asyncClient.multiget_slice(keysForThisClient, column_parent, predicate, consistencyLevel, LamportClock.sendTimestamp(), metadata, callback);
        }
...
/*
// more rounds for microbenchmark	
for (int i = 0; i < 3; ++i) {
firstRoundCallbacks.clear();
//System.err.println("2nd round, server size: " + asyncClientToFirstRoundKeys.size());
for (Entry<Cassandra.AsyncClient, List<ByteBuffer>> entry : asyncClientToFirstRoundKeys.entrySet()) {
            Cassandra.AsyncClient asyncClient = entry.getKey();
            List<ByteBuffer> keysForThisClient = entry.getValue();
//System.err.println("  key size: " + keysForThisClient.size());
        ...
        Map<ByteBuffer, List<ColumnOrSuperColumn>> keyToResult2 = new HashMap<ByteBuffer, List<ColumnOrSuperColumn>>();
        NavigableMap<Long, List<ByteBuffer>> lvtToKeys2 = new TreeMap<Long, List<ByteBuffer>>();
        for (BlockingQueueCallback<multiget_slice_call> callback : firstRoundCallbacks) {
            MultigetSliceResult result2 = callback.getResponseNoInterruption().getResult();
            //LamportClock.updateTime(result.lts);

            for (Entry<ByteBuffer, List<ColumnOrSuperColumn>> entry : result2.value.entrySet()) {
                ByteBuffer key = entry.getKey();
                List<ColumnOrSuperColumn> coscList = entry.getValue();
                keyToResult2.put(key, coscList);
            }
        }
}
//----------------------------
