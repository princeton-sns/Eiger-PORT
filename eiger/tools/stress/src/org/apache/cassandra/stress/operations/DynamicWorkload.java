...
public class DynamicWorkload extends Operation
{
    ...
    public void read(ClientLibrary clientLibrary, int columnsPerKey, int keysPerRead) throws IOException
    {
	    ...
	    Long latency = 0L;
		    ...
		    columnCount = 0;
/*
		    if (session.getExpandibleHop() > 0) {
                        // do expandible rotxn
                        results = clientLibrary.expandible_rotxn(keys, session.getExpandibleHop(), parent, nColumnsPredicate);
                    } else {
                        results = clientLibrary.transactional_multiget_slice(keys, parent, nColumnsPredicate);
                    }
                    //results = clientLibrary.transactional_multiget_slice(keys, parent, nColumnsPredicate, latency);
*/

		    if (session.getExpandibleHop() > 0) {
			results = new HashMap<ByteBuffer,List<ColumnOrSuperColumn>>();
			int numKeyPerHop = keys.size() / session.getExpandibleHop();
			Iterator<ByteBuffer> keyItr = keys.iterator();
                        List<ByteBuffer> subKeys = new ArrayList<ByteBuffer>();
                        while (keyItr.hasNext()) {
                            // construct sub key set for this hop
                            for (int i = 0; i < numKeyPerHop; ++i) {
                                subKeys.add(keyItr.next());
                            }
                            results.putAll(clientLibrary.transactional_multiget_slice(keys, parent, nColumnsPredicate));
                            subKeys.clear();
                        }
		    } else {
                        results = clientLibrary.transactional_multiget_slice(keys, parent, nColumnsPredicate);
